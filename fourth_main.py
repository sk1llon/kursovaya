...
from PyQt6.QtWidgets import QApplication, QWidget, QVBoxLayout, QFormLayout, QLineEdit, QPushButton, QTableWidget, \
    QTableWidgetItem, QDialog, QTabWidget, QComboBox, QDateEdit, QMessageBox, QTimeEdit, QLabel
...
def create_tables():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS doctors_conclusion (
                        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def add_appointment(patient_id, doctor_id, address, appointment_date, time):
    conn = sqlite3.connect('medical_records.db')
    cursor = conn.cursor()

    cursor.execute('''INSERT INTO appointments (patient_id, doctor_id, address, appointment_date, time)
                      VALUES (?, ?, ?, ?, ?)''', (patient_id, doctor_id, address, appointment_date, time))
...
class UserWindow(QDialog):
    def __init__(self, phone):
        ...
        self.services_table.setGeometry(10, 200, 100, 100)
...
class AnalysisMenu(QDialog):
    ...
    def save_analysis_appointment(self):
        patient_id = get_user(self.user_phone)
        services = get_services()
        service = services[self.row]
        ...
        add_analysis_appointments(patient_id[0], service[1], address, appointment_date, time)
...
class ConsultationMenu(QDialog):
    ...
    def load_combobox_data(self):
        doctors = get_doctor(self.service[1])

        self.doctor_combo.clear()
        self.address_combo.clear()
        for doctor in doctors:
            self.doctor_combo.addItem(f"{doctor[1]} {doctor[2]}")
            self.address_combo.addItem(f"{doctor[8]} {doctor[9]}")

    def save_appointment(self):
        patient_id = get_user(self.user_phone)
        doctor_id = self.doctor_combo.currentText()
        address = self.address_combo.currentText()
        appointment_date = self.appointment_date_input.date().toString("dd-MM-yyyy")
        time = self.time_input.text()
        ...
        add_appointment(patient_id[0], doctor_id, address, appointment_date, time)
...
class AddAppointmentTab(QWidget):
    ...
    def load_combobox_data(self):
        ...
        for doctor in doctors:
            self.doctor_combo.addItem(f"{doctor[1]} {doctor[2]} {doctor[8]} {doctor[9]}", doctor[0])
    ...
    def save_appointment(self):
        ...
        add_appointment(patient_id, doctor_id, address, appointment_date, time)
        self.close()
...
class ViewDataDialog(QDialog):
    def __init__(self):
        ...
        self.doctor_output_table = QTableWidget()
        ...
        self.appointment_table.setHorizontalHeaderLabels(['ID', 'Пациент', 'Врач', 'Адрес клиники', 'Дата приёма',
                                                          'Врема приёма', 'Кабинет'])

        self.doctor_output_table.setColumnCount(6)
        self.doctor_output_table.setHorizontalHeaderLabels(['ID', 'ID медицинской записи', 'Фамилия пациента',
                                                            'Фамилия врача', 'Дата приёма', 'Кабинет'])
        ...
        self.tabs.addTab(self.doctor_output_table, "Вывод врача")
